...
- (void)_createAttributeText {
    ...
    //自定义当前超链接文本颜色
    if ([self.zrxLabelDelegate respondsToSelector:@selector(linkColorWithWXLabel:)]) {
        self.linkColor = [self.zrxLabelDelegate linkColorWithWXLabel:self];
    }
    
    //自定义当前超链接文本颜色
    if ([self.zrxLabelDelegate respondsToSelector:@selector(passColorWithWXLabel:)]) {
        self.passColor = [self.zrxLabelDelegate passColorWithWXLabel:self];
    }
    if (self.text == nil) {
        return;
    }
    ...
    //----------------------设置链接文本的颜色-------------------
    //判断当前链接文本表达式是否实现
    if ([self.zrxLabelDelegate respondsToSelector:@selector(contentsOfRegexStringWithWXLabel:)] && [self.zrxLabelDelegate contentsOfRegexStringWithWXLabel:self] != nil)
    {
        //获取所有的链接文本
...
- (NSArray *)imagesOfRegexStrArray
{
    ...
    if ([self.zrxLabelDelegate respondsToSelector:@selector(imagesOfRegexStringWithWXLabel:)]) {
        regex = [self.zrxLabelDelegate imagesOfRegexStringWithWXLabel:self];
    }
...
- (void)replaceImageText
{
    ...
    for (NSString *imageUrl in [self imagesOfRegexStrArray]) {
        ...
        //将获取到得表情汉字遍历出来
        for (NSDictionary* faceDic in arr) {
            NSString *faceName = [faceDic objectForKey:@"chs"];
            
            if ([faceName isEqualToString:imageUrl]) {
                imgName = [faceDic objectForKey:@"png"];
            }
        }
...
- (NSArray *)contentsOfRegexStrArray
{
    ...
    NSString *regex = [self.zrxLabelDelegate contentsOfRegexStringWithWXLabel:self];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (range.length == 0) {
        ...
        //判断当前代理方法是否实现
        if ([self.zrxLabelDelegate respondsToSelector:@selector(touchEndZRXLabel:withContext:)]) {
            //获取当前点击字符串
            NSString *context = [[self.attrString string] substringWithRange:range];
            //调用点击开始代理方法
            [self.zrxLabelDelegate touchEndZRXLabel:self withContext:context];
        }
    }
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    {
        //判断当前代理方法是否实现
        if ([self.zrxLabelDelegate respondsToSelector:@selector(touchBeginZRXLabel:withContext:)]) {
            //获取当前点击字符串
            NSString *context = [[self.attrString string] substringWithRange:range];
            //调用点击开始代理方法
            [self.zrxLabelDelegate touchBeginZRXLabel:self withContext:context];
        }
    }
...
{
    ZRXLabel *zrxLabel = [[ZRXLabel alloc] initWithFrame:CGRectMake(0, 0, width, 900)];
    zrxLabel.linespace = linespace;
    zrxLabel.font = [UIFont systemFontOfSize:fontSize];
    zrxLabel.text = text;
    
    return zrxLabel.textHeight;
}
